...
import { T, Ico, formatNGN, formatUSD, NGN_RATE, formatTime, CTA, GhostBtn } from "./BuyFlowShared";

export default function BuyFlowStep2({
  payAmount, receiveAmount, selectedAsset, expiryTime,
  finalRate, quoteData,
  nextStep, prevStep, isExpired, resetExpiry, breadcrumbs,
}) {
  const isUrgent = expiryTime <= 60;

  // Format the rate nicely: e.g. ₦1,648 / $ - BTC
  const rateLabel = finalRate > 0
    ? `${formatNGN(finalRate)} / $ - ${selectedAsset.symbol}`
    : "—";
      ...
      <div className="step-content" style={{padding:"0 52px 60px",background:T.white,maxWidth:860,margin:"0 auto",width:"100%"}}>
        ...
        <div style={{background:T.white,border:`1.5px solid ${T.border}`,borderRadius:20,overflow:"hidden",marginBottom:24}}>
          ...
          <div style={{padding:"18px 32px",display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(140px, 1fr))",gap:24,borderBottom:`1px solid ${T.border}`}}>
            ...
            <div>
              <p style={{fontSize:10,fontWeight:700,color:T.text3,textTransform:"uppercase",letterSpacing:"1px",marginBottom:4,fontFamily:"'DM Sans',sans-serif"}}>Delivery</p>
              <p style={{fontFamily:"'Sora',sans-serif",fontSize:14,fontWeight:700,color:T.green}}>✓ Internal Wallet</p>
            </div>
            {quoteData?.id && (
              <div>
                <p style={{fontSize:10,fontWeight:700,color:T.text3,textTransform:"uppercase",letterSpacing:"1px",marginBottom:4,fontFamily:"'DM Sans',sans-serif"}}>Quote ID</p>
                <p style={{fontFamily:"'Sora',sans-serif",fontSize:14,fontWeight:700,color:T.text}}>#{quoteData.id}</p>
              </div>
            )}
          </div>
          ...
          <div style={{padding:"24px 32px"}}>
            ...
            {[
              ...
              ["Estimated crypto amount", `${receiveAmount} ${selectedAsset.symbol}`],
              ["NGN amount", formatNGN(payAmount)],
            ].map(([label,value],i)=>(
            ...
            <div style={{display:"flex",alignItems:"flex-start",gap:10,background:T.blueLight,borderRadius:12,padding:"14px 16px",marginTop:16}}>
              <Ico.info/>
              <p style={{fontSize:13,color:"#1A3A8A",lineHeight:1.6,fontFamily:"'DM Sans',sans-serif"}}>Your crypto will be automatically deposited into your Cheeseball internal wallet once payment is confirmed. No external address needed.</p>
            </div>
          </div>
        </div>
        ...
        <div className="actions-wrap" style={{display:"flex",gap:12,alignItems:"center"}}>
          ...
          <CTA onClick={nextStep} style={{flex:2}}>Choose Payment <Ico.arrow/></CTA>
        </div>
