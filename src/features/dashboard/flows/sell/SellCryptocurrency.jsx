...
const StepIndicator = ({ step, total = 3 }) => (
  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 24 }}>
    {Array.from({ length: total }, (_, i) => {
      ...
      return (
        <div key={i} style={{
          height: 6, flex: s === step ? 2 : 1, borderRadius: 3,
          background: s < step ? T.green : s === step ? T.blue : T.border,
          transition: "all 0.35s cubic-bezier(0.4, 0, 0.2, 1)"
        }} />
      );
    })}
    <span style={{ fontSize: 11, color: T.text3, fontWeight: 600, whiteSpace: "nowrap", marginLeft: 4, textTransform: "uppercase", letterSpacing: "0.5px" }}>
      {step}/{total}
    </span>
  </div>
);
...
const BackButton = ({ onClick }) => (
  <button className="back-btn" onClick={onClick} style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", cursor: "pointer", color: T.text2, fontSize: 13, fontWeight: 600, padding: "6px 0", marginBottom: 12, transition: "color 0.15s" }}>
    <Ico.back /> Back
  </button>
);
...
function SellStep({ onContinue }) {
  ...
  const [cryptoSource, setCryptoSource] = useState("external_wallet");
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        let merged = assetsList
          .filter(a => (a.symbol || a.code) !== "NGN")
          .map((a, i) => {
            const symbol = a.symbol || a.code;
          const w = walletsList.find(w => w.asset === symbol);
          const meta = ASSET_ICONS[symbol] || { icon: symbol[0], color: COLORS[i % COLORS.length] };
          return {
            sym: symbol, name: a.name || symbol, icon: meta.icon,
            color: meta.color,
        ...
        if (merged.length === 0) {
          merged = ["BTC", "ETH", "USDT"].map((sym, i) => {
            ...
            return { sym, name: sym, icon: meta.icon, color: meta.color, bal: w ? (w.available_balance || w.balance || 0) : 0 };
          });
        }
        setCoins(merged);
  ...
  const fetchRate = useCallback(async () => {
    ...
    try {
      let q;
      if (inputMode === "fiat") {
        q = await getSellQuote(coin.sym, null, v);
      } else {
        q = await getSellQuote(coin.sym, v);
      }
  ...
  const handleModeSwitch = (newMode) => {
    ...
    if (quote && v > 0) {
      if (newMode === "fiat") {
        setAmount(String(Math.round(quote.naira_amount * 100) / 100));
      } else {
        setAmount(String(quote.crypto_amount));
      }
    } else {
      setAmount("");
    }
  ...
  const balanceExceeded = cryptoSource === "cheeseball_wallet" && inputMode === "crypto" && v > coin.bal;

  const equivalentText = quote
    ? (inputMode === "crypto" ? `≈ ${fmtNGN(quote.naira_amount)}` : `≈ ${fmtCoin(quote.crypto_amount, coin.sym)}`)
    : null;

  const sysFee = quote ? (quote.market_rate * (quote.markup_percent / 100)) * quote.crypto_amount * quote.crypto_usd_price : 0;

  return (
    <div className="sell-grid" style={{ display: "grid", gridTemplateColumns: "1fr 400px", minHeight: "100%", background: T.white, overflowX: "hidden", maxWidth: "100vw" }}>
      <div style={{ padding: "44px 52px 60px", borderRight: `1px solid ${T.border}` }}>
        ...
        <p style={{ fontSize: 14, color: T.text2, marginTop: 6, lineHeight: 1.6 }}>Select an asset, choose your source, and enter an amount.</p>

        {/* ── Wallet Source Cards ── */}
        <p style={{ fontSize: 11, fontWeight: 600, color: T.text3, textTransform: "uppercase", letterSpacing: "0.8px", marginTop: 28, marginBottom: 10 }}>Crypto source</p>
        <div className="source-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          {[
            { key: "external_wallet", label: "External Wallet", sub: "Send from Trust Wallet, Binance, Bybit, etc.", IconComp: Ico.external },
            { key: "cheeseball_wallet", label: "Cheeseball Wallet", sub: "Use your in-app balance to sell instantly", IconComp: Ico.wallet },
          ].map(({ key, label, sub, IconComp }) => {
            const active = cryptoSource === key;
            return (
              <button key={key} className="source-card" onClick={() => setCryptoSource(key)}
                style={{ border: `1.5px solid ${active ? T.blue : T.border}`, borderRadius: 16, padding: "20px 18px", background: active ? T.blueLight : T.white, cursor: "pointer", textAlign: "left", transition: "all 0.18s", position: "relative" }}>
                ...
                <div style={{ width: 40, height: 40, borderRadius: 12, background: active ? T.blue : T.surface, display: "flex", alignItems: "center", justifyContent: "center", color: active ? "#fff" : T.text2, marginBottom: 12, transition: "all 0.18s" }}><IconComp /></div>
                <p style={{ fontFamily: "'Sora', sans-serif", fontSize: 14, fontWeight: 700, color: active ? T.blue : T.text, marginBottom: 4 }}>{label}</p>
                <p style={{ fontSize: 12, color: active ? "#3B5AA8" : T.text3, lineHeight: 1.45 }}>{sub}</p>
                {active && key === "cheeseball_wallet" && (
                  <div style={{ marginTop: 10, paddingTop: 10, borderTop: `1px solid ${T.blue}22`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <span style={{ fontSize: 11, color: "#3B5AA8", fontWeight: 500 }}>Available</span>
                    <span style={{ fontFamily: "'Sora', sans-serif", fontSize: 13, fontWeight: 700, color: T.blue }}>{fmtCoin(coin.bal, coin.sym)}</span>
                  </div>
                )}
              </button>
            );
                ...
                {coins.map((c) => (
                  <div key={c.sym} className="ddopt" style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 10px", borderRadius: 12, cursor: "pointer", background: c.sym === coin.sym ? T.blueLight : "transparent", transition: "background 0.12s" }} onClick={() => selectCoin(c)}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontFamily: "'Sora', sans-serif", fontSize: 13, fontWeight: 600, color: T.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{c.name}</p>
                      <p style={{ fontSize: 11, color: T.text2, marginTop: 1 }}>{c.sym}</p>
                    </div>
                    {c.sym === coin.sym && <span style={{ color: T.blue, fontSize: 15, marginLeft: "auto", flexShrink: 0 }}>✓</span>}
                  </div>
                ))}
              </div>
            )}
        ...
        <div style={{ marginTop: 28 }}>
          ...
          {/* Segmented control */}
          <div style={{ display: "flex", background: T.surface, borderRadius: 12, padding: 3, marginBottom: 12, border: `1px solid ${T.border}` }}>
            {[
              { key: "crypto", label: coin.sym },
              { key: "fiat", label: "NGN (₦)" },
            ].map(({ key, label }) => {
              const active = inputMode === key;
              return (
                <button key={key} className="seg-btn" onClick={() => handleModeSwitch(key)}
                  style={{ flex: 1, padding: "10px 16px", borderRadius: 10, border: "none", cursor: "pointer", fontFamily: "'Sora', sans-serif", fontSize: 13, fontWeight: 700, color: active ? T.blue : T.text3, background: active ? T.white : "transparent", boxShadow: active ? "0 2px 8px rgba(10,15,30,0.06)" : "none", transition: "all 0.2s" }}>
                  {label}
                </button>
              );
            })}
          ...
          <div style={{ border: `1.5px solid ${focused ? T.blue : (balanceExceeded ? "#EF4444" : T.border)}`, borderRadius: 16, padding: "20px 22px", background: T.white, transition: "border-color 0.18s" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <span style={{ fontFamily: "'Sora', sans-serif", fontSize: 28, fontWeight: 700, color: T.text3, flexShrink: 0 }}>{inputMode === "fiat" ? "₦" : ""}</span>
              <input className="sell-amt-input"
                style={{ flex: 1, width: "100%", border: "none", outline: "none", minWidth: 0, fontFamily: "'Sora', sans-serif", fontSize: 36, fontWeight: 700, color: v > 0 ? T.text : "#CED6E8", background: "transparent", letterSpacing: "-1.5px" }}
              ...
              />
              <div style={{ background: T.blueLight, borderRadius: 10, padding: "8px 13px", flexShrink: 0 }}>
                <span style={{ fontFamily: "'Sora', sans-serif", fontSize: 13, fontWeight: 700, color: T.blue }}>{inputMode === "crypto" ? coin.sym : "NGN"}</span>
              </div>
            </div>

            {/* Equivalent value */}
            {equivalentText && rateStatus === "live" && (
              <p className="fadein" style={{ fontSize: 13, color: T.text2, marginTop: 10, fontWeight: 500 }}>{equivalentText}</p>
            )}
          ...
          {balanceExceeded && (
            <div className="fadein" style={{ marginTop: 10, padding: "10px 14px", background: "#FEF2F2", borderRadius: 10, display: "flex", alignItems: "center", gap: 8 }}>
              <Ico.warn />
              <span style={{ fontSize: 12, color: "#B91C1C", fontWeight: 500 }}>Amount exceeds your available balance of {fmtCoin(coin.bal, coin.sym)}</span>
            </div>
          )}

          {/* Max button for cheeseball wallet */}
          {cryptoSource === "cheeseball_wallet" && inputMode === "crypto" && (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(70px, 1fr))", gap: 8, marginTop: 10 }}>
              <button className="qbtn" style={{ flex: "1 0 60px", border: `1.5px solid ${T.border}`, background: T.white, borderRadius: 10, padding: "8px 10px", fontSize: 12, fontWeight: 600, color: T.text2, cursor: "pointer", transition: "all 0.15s", textAlign: "center" }}
                onClick={() => setAmount(coin.bal.toString())}>Max ({fmtCoin(coin.bal, coin.sym)})</button>
            </div>
          )}

          {/* Success summary */}
          {v > 0 && rateStatus === "live" && quote && !balanceExceeded && (
        ...
        <button className={canContinue && !balanceExceeded ? "ctaon" : ""} disabled={!canContinue || balanceExceeded}
          onClick={() => canContinue && !balanceExceeded && onContinue({ coin, amount: v, cryptoSource, quote, netNGN: quote.naira_amount, ngnRate: quote.final_rate, sysFee, usdValue: quote.crypto_amount * quote.crypto_usd_price })}
          style={{ width: "100%", marginTop: 24, padding: "17px", borderRadius: 14, border: "none", fontFamily: "'Sora', sans-serif", fontSize: 15, fontWeight: 700, cursor: canContinue && !balanceExceeded ? "pointer" : "not-allowed", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, transition: "all 0.2s", background: canContinue && !balanceExceeded ? T.blue : "#E8EEFF", color: canContinue && !balanceExceeded ? "#fff" : "#A8B4CC" }}>
      ...
      <div style={{ padding: "44px 32px 60px", background: T.surface, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ background: T.blue, borderRadius: 16, padding: "18px 20px", marginBottom: 14 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 14, paddingTop: 14, borderTop: "1px solid rgba(255,255,255,0.15)" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ fontSize: 12, color: "rgba(255,255,255,0.55)" }}>Source</span><span style={{ fontFamily: "'Sora', sans-serif", fontSize: 12, fontWeight: 700, color: "rgba(255,255,255,0.9)" }}>{cryptoSource === "cheeseball_wallet" ? "Cheeseball" : "External"}</span></div>
          </div>
        </div>
        {rateStatus === "live" && v > 0 && (
...
function PayoutStep({ order: s, onContinue, onBack, onNavigate }) {
  ...
  useEffect(() => {
    getBeneficiaries()
      .then(res => {
        setBanks(res.map(b => ({
          id: b.id, name: b.account_name, bank: b.bank_name, number: b.account_number
        })));
      })
      .catch(() => setBanks([]))
    ...
    <div className="sell-grid" style={{ display: "grid", gridTemplateColumns: "1fr 400px", minHeight: "100%", background: T.white, overflowX: "hidden", maxWidth: "100vw" }}>
      <div style={{ padding: "44px 52px 60px", borderRight: `1px solid ${T.border}` }}>
        ...
        <div className="method-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginTop: 32 }}>
          {[
            { key: "bank", label: "Bank Account", sub: "Send to your registered bank", Icon: Ico.bank },
            { key: "wallet", label: "NGN Wallet", sub: "Credit your in-app wallet", Icon: Ico.wallet },
          ...
          <div className="fadein" style={{ marginTop: 24 }}>
            ...
            {banksLoading ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {[1, 2].map(i => <div key={i} className="pulsing" style={{ height: 72, borderRadius: 14, background: T.surface, border: `1px solid ${T.border}` }} />)}
              </div>
            ) : banks.length === 0 ? (
              /* Empty state */
              <div style={{ textAlign: "center", padding: "32px 20px", border: `1.5px dashed ${T.border}`, borderRadius: 16, background: T.surface }}>
                <div style={{ width: 48, height: 48, borderRadius: 14, background: T.blueLight, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 14px", color: T.blue }}><Ico.bank /></div>
                <p style={{ fontFamily: "'Sora', sans-serif", fontSize: 14, fontWeight: 700, color: T.text, marginBottom: 4 }}>No bank accounts yet</p>
                <p style={{ fontSize: 13, color: T.text2, lineHeight: 1.5, marginBottom: 16 }}>Add a bank account to receive your payout.</p>
                <button className="ctaon" onClick={() => onNavigate?.("bank-accounts")} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "11px 20px", borderRadius: 12, border: "none", background: T.blue, color: "#fff", fontFamily: "'Sora', sans-serif", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                  Add Bank Account <Ico.chevron />
                </button>
              </div>
            ) : (
              /* Bank list */
              <>
                  ...
                  {banks.map((acc) => {
                    ...
                    return (
                      <div key={acc.id} className="bank-row" onClick={() => setSelectedBank(acc)} style={{ display: "flex", alignItems: "center", gap: 14, padding: "16px 18px", border: `1.5px solid ${sel ? T.blue : T.border}`, borderRadius: 14, cursor: "pointer", background: sel ? T.blueLight : T.white, transition: "all 0.15s" }}>
                        <div style={{ width: 42, height: 42, borderRadius: 12, background: sel ? T.blue : T.surface, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, color: sel ? "#fff" : T.text2 }}><Ico.bank /></div>
                        <div style={{ flex: 1 }}>
                          <p style={{ fontFamily: "'Sora', sans-serif", fontSize: 14, fontWeight: 700, color: sel ? T.blue : T.text }}>{acc.name}</p>
                          <p style={{ fontSize: 12, color: sel ? "#3B5AA8" : T.text2, marginTop: 2 }}>{acc.bank} · {acc.number}</p>
                        </div>
                ...
                <button className="manage-link" onClick={() => onNavigate?.("bank-accounts")} style={{ display: "flex", alignItems: "center", gap: 6, margin: "14px auto 0", background: "none", border: "none", cursor: "pointer", fontSize: 13, fontWeight: 600, color: T.blue, transition: "opacity 0.15s" }}>
                  Manage bank accounts <Ico.chevron />
                </button>
              </>
            )}
          </div>
        ...
        {method === "wallet" && (
          <div className="fadein" style={{ marginTop: 24 }}>
            <div style={{ display: "flex", alignItems: "flex-start", gap: 12, background: T.blueLight, borderRadius: 14, padding: "16px 18px" }}>
              <Ico.info />
              <div>
                <p style={{ fontFamily: "'Sora', sans-serif", fontSize: 13, fontWeight: 700, color: T.blue, marginBottom: 4 }}>Instant credit</p>
                <p style={{ fontSize: 13, color: "#3B5AA8", lineHeight: 1.55 }}>Your NGN wallet will be credited automatically after the transaction is confirmed.</p>
              </div>
            </div>
          </div>
        )}
      ...
      <div style={{ padding: "44px 32px 60px", background: T.surface, display: "flex", flexDirection: "column" }}>
        ...
        {method && (
          <div className="fadein" style={{ background: T.white, border: `1.5px solid ${T.border}`, borderRadius: 16, padding: "16px 18px", marginBottom: 14 }}>
            ...
            {method === "wallet" ? (
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}><div style={{ width: 34, height: 34, borderRadius: 10, background: T.greenLight, display: "flex", alignItems: "center", justifyContent: "center", color: T.green }}><Ico.wallet /></div><div><p style={{ fontFamily: "'Sora', sans-serif", fontSize: 13, fontWeight: 700, color: T.text }}>NGN Wallet</p><p style={{ fontSize: 12, color: T.text2, marginTop: 1 }}>Instant credit on confirmation</p></div></div>
            ) : selectedBank ? (
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}><div style={{ width: 34, height: 34, borderRadius: 10, background: T.blueLight, display: "flex", alignItems: "center", justifyContent: "center", color: T.blue }}><Ico.bank /></div><div><p style={{ fontFamily: "'Sora', sans-serif", fontSize: 13, fontWeight: 700, color: T.text }}>{selectedBank.name}</p><p style={{ fontSize: 12, color: T.text2, marginTop: 1 }}>{selectedBank.bank} · {selectedBank.number}</p></div></div>
...
/* ═══════════════════════════════════════════════════════ */
/* ── STEP 3: CONFIRM ── */
function ConfirmStep({ order: o, onConfirm, onBack }) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setToast({ message: e.message || "Failed to create transaction. Please try again.", type: "error" });
      setLoading(false);
    }
  };
  ...
  return (
    <>
      <div className="sell-grid" style={{ display: "grid", gridTemplateColumns: "1fr 420px", minHeight: "100%", background: T.white, overflowX: "hidden", maxWidth: "100vw" }}>
        <div style={{ padding: "44px 52px 60px", borderRight: `1px solid ${T.border}` }}>
          <nav style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 20, flexWrap: "wrap" }}>
          ...
          <div style={{ marginTop: 32, border: `1.5px solid ${T.border}`, borderRadius: 20, background: T.white, overflow: "hidden" }}>
            ...
            <div style={{ padding: "0 24px" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "13px 0", borderBottom: `1px solid ${T.border}` }}><span style={{ fontSize: 13, color: T.text2 }}>Payout method</span><span style={{ fontFamily: "'Sora', sans-serif", fontSize: 13, fontWeight: 600, color: T.text, display: "flex", alignItems: "center", gap: 6 }}>{isBank ? <Ico.bank /> : <Ico.wallet />}{isBank ? "Bank Account" : "NGN Wallet"}</span></div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "13px 0", borderBottom: `1px solid ${T.border}` }}><span style={{ fontSize: 13, color: T.text2 }}>{isBank ? "Paid to" : "Credited to"}</span><span style={{ fontFamily: "'Sora', sans-serif", fontSize: 13, fontWeight: 600, color: T.text }}>{isBank ? `${o.bank.name} · ${o.bank.bank}` : "NGN Wallet"}</span></div>
        ...
        <div style={{ padding: "44px 32px 60px", background: T.surface, display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ background: T.white, border: `1.5px solid ${T.border}`, borderRadius: 20, overflow: "hidden" }}>
            ...
            <div style={{ padding: "16px 22px 22px" }}>
              ...
              {isBank ? (
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}><div style={{ width: 38, height: 38, borderRadius: 10, background: T.blueLight, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, color: T.blue }}><Ico.bank /></div><div><p style={{ fontFamily: "'Sora', sans-serif", fontSize: 13, fontWeight: 700, color: T.text }}>{o.bank.name}</p><p style={{ fontSize: 12, color: T.text2, marginTop: 2 }}>{o.bank.bank}</p><p style={{ fontFamily: "'Sora', sans-serif", fontSize: 13, fontWeight: 600, color: T.text, marginTop: 1 }}>{o.bank.number}</p></div></div>
              ) : <div style={{ display: "flex", alignItems: "center", gap: 12 }}><div style={{ width: 38, height: 38, borderRadius: 10, background: T.greenLight, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, color: T.green }}><Ico.wallet /></div><div><p style={{ fontFamily: "'Sora', sans-serif", fontSize: 13, fontWeight: 700, color: T.text }}>NGN Wallet</p><p style={{ fontSize: 12, color: T.text2, marginTop: 2 }}>Instant credit</p></div></div>}
            </div>
          </div>
        </div>
      </div>
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;font-family:'DM Sans',sans-serif;}
        ...
        .source-card:hover{border-color:${T.blue}!important;}
        .bank-row:hover{background:${T.surface}!important;}
        .back-btn:hover{color:${T.blue}!important;}
        .manage-link:hover{opacity:0.75!important;}
        .seg-btn:hover{color:${T.blue}!important;}
      ...
      {step === 2 && <PayoutStep order={order} onBack={goBack} onNavigate={onNavigate} onContinue={(data) => { setOrder(prev => ({ ...prev, ...data })); setStep(3); }} />}
      {step === 3 && <ConfirmStep order={order} onBack={goBack} onConfirm={(apiResp) => { setOrder(prev => ({ ...prev, submittedAt: new Date(), transaction: apiResp })); if (order.cryptoSource === "external_wallet") { setStep(4); } else { setStep(5); } }} />}
      {step === 4 && <SendCryptoStep order={order} onSent={() => setStep(5)} />}
      {step === 5 && <PaymentSubmittedStep order={order} onGoHome={() => onNavigate?.("dashboard")} onContactSupport={() => onNavigate?.("support")} />}
    </>
  );
}
