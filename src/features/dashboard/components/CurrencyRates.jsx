...
const QUICK_ACTIONS = [
  { label: "Deposit",     icon: ArrowDownLeft,    color: T.blue,    bg: T.blueLight,   page: "deposit"          },
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", display: "flex", flexDirection: "column", gap: 24 }}>
                ...
                <div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 8, alignItems: "center" }}>
                    ...
                    <button
                      ...
                      style={{ width: 36, height: 36, borderRadius: 10, background: "rgba(255,255,255,0.12)", border: "1px solid rgba(255,255,255,0.15)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}
                    >
                      {balanceVisible ? <Eye size={16} color="rgba(255,255,255,0.8)" /> : <EyeOff size={16} color="rgba(255,255,255,0.8)" />}
                    </button>
                  </div>
                </div>
        ...
        <div className="quick-actions-grid" style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 14 }}>
          {QUICK_ACTIONS.map((qa) => (
            <button
              ...
              style={{ background: T.white, border: `1.5px solid ${T.border}`, borderRadius: 20, padding: "22px 12px", display: "flex", flexDirection: "column", alignItems: "center", gap: 12, cursor: "pointer", transition: "all 0.18s", boxShadow: "none" }}
            >
              <div style={{ width: 48, height: 48, borderRadius: 14, background: qa.bg, display: "flex", alignItems: "center", justifyContent: "center", transition: "transform 0.18s" }}>
                <qa.icon size={20} color={qa.color} strokeWidth={2} />
              </div>
              <span style={{ fontSize: 11, fontWeight: 700, color: T.text, textTransform: "uppercase", letterSpacing: "0.6px", textAlign: "center" }}>{qa.label}</span>
            </button>
          ))}
        </div>
        ...
        <div className="dash-grid-bottom" style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: 20, alignItems: "start" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            {/* Recent transactions */}
            <div style={{ background: T.white, borderRadius: 24, border: `1.5px solid ${T.border}`, padding: "22px 24px" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
                <h2 style={{ fontFamily: "'Sora', sans-serif", fontSize: 15, fontWeight: 700, color: T.text, letterSpacing: "-0.3px" }}>Recent Activity</h2>
                <button
                  onClick={() => onNavigate("history")}
                  style={{ width: 32, height: 32, borderRadius: 9, background: T.surface, border: `1px solid ${T.border}`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: T.text2, transition: "all 0.15s" }}
                ...
                {transactions.map((txn) => {
                  ...
                  return (
                    <div
                      key={txn.id}
                      className="txn-row"
                      style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "11px 10px", borderRadius: 12, cursor: "pointer", transition: "background 0.12s" }}
                    >
                      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        <div style={{ width: 38, height: 38, borderRadius: 11, background: T.surface, display: "flex", alignItems: "center", justifyContent: "center", position: "relative", flexShrink: 0 }}>
                          {txn.positive
                            ? <ArrowDownLeft size={16} color={T.green} />
                            : <ArrowUpRight size={16} color={T.text3} />
                          }
                          <div style={{ position: "absolute", bottom: -1, right: -1, width: 10, height: 10, borderRadius: "50%", background: cfg.bg, border: `1.5px solid ${cfg.border}` }} />
                        </div>
                        <div>
                          <p style={{ fontFamily: "'Sora', sans-serif", fontSize: 13, fontWeight: 700, color: T.text, marginBottom: 2 }}>{txn.type}</p>
                          <p style={{ fontSize: 11, color: T.text3, fontWeight: 500 }}>{txn.date}</p>
                        </div>
                      </div>
                      <div style={{ textAlign: "right" }}>
                        <p style={{ fontFamily: "'Sora', sans-serif", fontSize: 13, fontWeight: 700, color: txn.positive ? T.greenText : T.text, marginBottom: 3 }}>{txn.amount}</p>
                        <span style={{ fontSize: 10, fontWeight: 700, color: cfg.color, background: cfg.bg, padding: "2px 7px", borderRadius: 6, textTransform: "uppercase", letterSpacing: "0.4px" }}>
                          {cfg.label}
                        </span>
                      </div>
                    </div>
            ...
            {/* Market card */}
            <div style={{ background: T.blue, borderRadius: 24, padding: "28px 24px", position: "relative", overflow: "hidden", cursor: "pointer" }}>
              ...
              <div style={{ position: "relative", zIndex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 5, background: "rgba(255,255,255,0.12)", borderRadius: 20, padding: "5px 12px" }}>
                    <div className="blink" style={{ width: 6, height: 6, borderRadius: "50%", background: T.mintGreen }} />
                    <span style={{ fontSize: 10, fontWeight: 700, color: "#fff", textTransform: "uppercase", letterSpacing: "0.5px" }}>Market +2.4%</span>
                  </div>
                </div>
                <h3 style={{ fontFamily: "'Sora', sans-serif", fontSize: 20, fontWeight: 700, color: "#fff", lineHeight: 1.2, marginBottom: 6 }}>
                  Global Market Rates
                </h3>
                <p style={{ fontSize: 12, color: "rgba(255,255,255,0.5)", marginBottom: 20, fontWeight: 500 }}>Real-time data for 200+ pairs</p>

                {/* Market rows */}
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {filteredMarket.slice(0, 3).map((m) => (
                    <div key={m.symbol} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", background: "rgba(255,255,255,0.08)", borderRadius: 12, border: "1px solid rgba(255,255,255,0.08)" }}>
                      <div style={{ width: 30, height: 30, borderRadius: "50%", background: m.color, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Sora', sans-serif", fontSize: 11, fontWeight: 700, color: "#fff", flexShrink: 0 }}>{m.icon}</div>
                      <div style={{ flex: 1 }}>
                        <p style={{ fontFamily: "'Sora', sans-serif", fontSize: 12, fontWeight: 700, color: "#fff" }}>{m.symbol}</p>
                        <p style={{ fontSize: 10, color: "rgba(255,255,255,0.45)", fontWeight: 500 }}>{m.name}</p>
                      </div>
                      <div style={{ textAlign: "right" }}>
                        <p style={{ fontFamily: "'Sora', sans-serif", fontSize: 12, fontWeight: 700, color: "#fff" }}>{m.price}</p>
                        <p style={{ fontSize: 10, fontWeight: 700, color: m.change >= 0 ? T.mintGreen : "#FCA5A5" }}>
                          {m.change >= 0 ? "+" : ""}{m.change}%
                        </p>
                      </div>
                    </div>
